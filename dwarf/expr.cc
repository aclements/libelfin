...
#define CHECKN(n) do { if (stack.size() < n) goto underflow; } while (0)
            ...
            static_assert(sizeof(tmp1) == sizeof(taddr),
                          "taddr is not 64 bits");
            ...
            switch (op) {
                ...
                case DW_OP::deref_size:
                    ...
                    if (tmp1.u > subsec->addr_size)
                        throw expr_error(
                                "DW_OP_deref_size operand exceeds address size");
                deref_common:
                    CHECK();
                ...
                case DW_OP::xderef_size:
                    ...
                    if (tmp1.u > subsec->addr_size)
                        throw expr_error(
                                "DW_OP_xderef_size operand exceeds address size");
                xderef_common:
                    CHECKN(2);
                    tmp2.u = stack.back();
                    stack.pop_back();
                    stack.back() = ctx->xderef_size(tmp2.u, stack.back(),
                                                    tmp1.u);
                    break;
                case DW_OP::push_object_address:
                    // XXX
                    throw runtime_error(
                            "DW_OP_push_object_address not implemented");
...
#define SRELOP(relop)                                                   \
                ...
                skip_common:
                    cur = cursor(subsec,
                                 (int64_t) cur.get_section_offset() + tmp1.s);
                    break;
                case DW_OP::call2:
                case DW_OP::call4:
                case DW_OP::call_ref:
